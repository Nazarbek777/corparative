...
@section('content')
    <form action="{{ route('testimonials.store') }}" method="POST" enctype="multipart/form-data" novalidate class="needs-validation" onsubmit="updateEditorContent()">
        @csrf
                ...
                <div class="page-header">
                    <div class="page-header-left d-flex align-items-center">
                        <div class="page-header-title">
                            <h5 class="m-b-10">Создать Отзыв</h5>
                        </div>
                        <ul class="breadcrumb">
                                            ...
                                            <div class="tab-pane fade show {{ $lang == 'uz' ? 'active' : '' }}" id="{{ $lang }}Content">
                                                <div class="form-group pb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="name_{{ $lang }}" name="name_{{ $lang }}"
                                                           value="{{ old('name_' . $lang, $news->{'name_' . $lang} ?? '') }}" required>
                                                </div>
                                                <div class="form-group pb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="position_{{ $lang }}" name="position_{{ $lang }}"
                                                           value="{{ old('position_' . $lang, $news->{'position_' . $lang} ?? '') }}" required>
                                                </div>
                                                <div class="form-group pb-3">
                                                    <label for="testimonial_{{ $lang }}">Отзыв ({{ strtoupper($lang) }}):</label>
                                                    <div id="testimonialEditor_{{ $lang }}" style="height:200px;">{!! old('testimonial_' . $lang, $news->{'testimonial_' . $lang} ?? '') !!}</div>
                                                    <input type="hidden" id="testimonial_{{ $lang }}" name="testimonial_{{ $lang }}" value="{{ old('testimonial_' . $lang, $news->{'testimonial_' . $lang} ?? '') }}">
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
